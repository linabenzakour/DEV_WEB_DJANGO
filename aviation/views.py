    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                # Si l'aéroport est plein, rediriger vers la page d'erreur
                return redirect('aeroport_plein')  # Redirige vers la vue d'erreur
    else:
        form = AvionForm()
...
def ajouter_avion(request):
    if request.method == "POST":
        ...
        try:
            ...
            # Création et sauvegarde de l'avion
            avion = Avion(id_avion=id_avion, compagnie_aerienne=compagnie, lieu=aeroport)
            avion.save()

            return redirect("data_list")  # Redirection vers la liste des données après succès
    ...
    # Si GET, afficher le formulaire avec tous les aéroports
    aeroports = Aeroport.objects.all()
...
def create_aeroport(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            aeroport = form.save()  # Le champ id_aeroport est généré automatiquement
...
def modify_aeroport(request, id_aeroport):
    aeroport = get_object_or_404(Aeroport, id_aeroport=id_aeroport)  # Recherche l'aéroport par son id
    if request.method == 'POST':
        form = AeroportForm(request.POST, request.FILES, instance=aeroport)
        if form.is_valid():
            form.save()  # Les modifications sont appliquées à l'aéroport existant
            return redirect('detail_aeroport', id_aeroport=aeroport.id_aeroport)  
    else:
